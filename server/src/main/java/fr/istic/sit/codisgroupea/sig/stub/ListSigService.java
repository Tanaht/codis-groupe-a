...
public class ListSigService implements SigRepository<SymbolSitac> {
    ...
    public ListSigService() {
        ...
        addToSig(Color.BLUE, Shape.WATERRESOURCE, null,
                new Position(48.11540, -1.63813));
        addToSig(Color.ORANGE, Shape.DANGERDOWN, null,
                new Position(48.11520, -1.63999));
    }
     ...
     */
    private void addToSig(Color color,
                          Shape shape,
                          Payload payload,
                          Position location) {
    ...
    @Override
    public SymbolSitac createSymbolFromSigEntry(Intervention intervention,
                                                SymbolSitac entry) {
    ...
    @Override
    public List<SymbolSitac> getInterventionSymbols(Intervention intervention) {
        ...
        for (SymbolSitac symbolSitac : listSymbolBouchon) {
            val distance = getMeters(
                    intervention.getPosition(),
                    symbolSitac.getLocation());

            if (distance <= sizeFromTheInterventionCenter) {
                val copySymbolSitac = new SymbolSitac();
    ...
    private double getMeters(Position pos1, Position pos2) {
        ...
        double partialResult2 = atan2(
                sqrt(partialResult1),
                sqrt(1 - partialResult1));
